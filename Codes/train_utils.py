...
def train(source_BHP, Qinj, Tmap, p0, sw0, n_iters, learning_rate,
    ...
    for k in range(tsteps):
        ...
        model_save_path = '/scratch/user/jungangc/PICNN-2phase/PICNN-2phaseflow-constBHP-64by64-heter-TransferLearning-50stepsby2-final/Codes/model/checkpoint2000_{}.pt'.format(
            k)

        model = PhyConvNet(num_classes=1, in_channels=2, time_sim=tsteps).cuda()
        if k > 0:
            prev_model_save_path = '/scratch/user/jungangc/PICNN-2phase/PICNN-2phaseflow-constBHP-64by64-heter-TransferLearning-50stepsby2-final/Codes/model/checkpoint2000_{}.pt'.format(
                k - 1)
        ...
        while monitor_loss > 0.001 and epoch < 50000:
            ...
            # print(tsteps)
            loss = lossfunc.compute_loss(p_prev, sw_prev, xp, xs, loss_physics, dt, Rate_vec[:, k:k + 1],
                                         BHP_vec[:, k:k + 1], Perm, sim_step=steps_sim)
...
def trainTriplex(source_BHP, Qinj, Tmap, p0, sw0, n_iters, learning_rate,
                 dt, Rate_vec, BHP_vec, Perm, tsteps, steps_sim, input):
    train_loss_list = []
    loss_physics = loss_generator(dt)
    outputs = []
    outputs.append(torch.cat((p0, sw0), dim=1))
    for k in range(tsteps):
        # pre_model_save_path = '/scratch/user/jungangc/PICNN-2phase/PICNN-2phaseflow-constBHP-64by64-homo-Progressive-10steps-final/Codes/model/checkpoint1000_{}.pt'.format(k)
        model_save_path = 'model/checkpoint2000_{}.pt'.format(
            k)

        model = PhyConvNet(num_classes=1, in_channels=3, time_sim=tsteps).cuda()
        if k > 0:
            prev_model_save_path = 'model/checkpoint2000_{}.pt'.format(k - 1)
            checkpoint = torch.load(prev_model_save_path)
            model.load_state_dict(checkpoint['model_state_dict'])
        ...
        while monitor_loss > 0.001 and epoch < 50000:
            ...
            batch_loss = 0
            # output is a list
            xp = model(p_prev, input)

            # get loss

            lossfunc = CustomLoss(initial_coefficient=[1.0, 1.0])
            # print(tsteps)
            loss = lossfunc.compute_loss(p_prev, input)
